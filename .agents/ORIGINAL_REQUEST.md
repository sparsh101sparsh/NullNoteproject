...
## 2026-06-15T18:01:33Z

<USER_REQUEST>
Add `/h` (Marker) and `/p` (Screenshot) slash commands to the NullNote text editor inside the sidepanel. Also update the editor's placeholder text to show all keyboard shortcuts and slash commands.
...
### R1. Slash Commands in Editor
Inside the contentEditable editor, typing `/h` should trigger the quick highlight (marker) action, and typing `/p` should trigger the quick screenshot action.
- The command must trigger **whenever** `/h` or `/p` is typed anywhere in the text (e.g. even inside a word/string like `hello/h`).
- The typed command (the `/` character and the `h` or `p` character) must be automatically deleted from the editor text so that the notes remain clean.
- The caret position must be correctly handled after the command text deletion.

### R2. Updated Placeholder Text
Update the editor's empty placeholder text to list all shortcuts and slash commands on multiple lines with clean formatting:
```
...
```
The style of the placeholder must support displaying multiple lines (e.g., using `whiteSpace: 'pre-wrap'` or separate lines with line breaks).

## Acceptance Criteria

### Slash Commands Verification
- [ ] Typing `/h` in the editor triggers the marker function and deletes `/h` from the editor text.
- [ ] Typing `/p` in the editor triggers the screenshot function and deletes `/p` from the editor text.
- [ ] Caret/cursor position is restored correctly after deleting the command text.

### Placeholder Verification
- [ ] The placeholder text displays the updated list of shortcuts arranged on multiple lines with bullet/hyphen pointers when the editor is completely empty.
</USER_REQUEST>

## 2026-06-15T21:12:29Z
